from poll_up import poll_up, db, celery
...
class Testpoll_up():
    ...
    @classmethod
    def setUpClass(cls):
        poll_up.config['DEBUG'] = False
        poll_up.config['TESTING'] = True
        cls.DB_PATH = os.path.join(os.path.dirname(__file__), 'poll_up_test.db')
        poll_up.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///{}'.format(cls.DB_PATH)
        ...
        with poll_up.app_context():
            db.init_app(poll_up)
            db.create_all()
            cls.p = Process(target=poll_up.run, kwargs={'port': 7000})
            cls.p.start()
            time.sleep(2)
    ...
    def test_celery_task(self):
        ...
        result = close_poll.apply((1, poll_up.config['SQLALCHEMY_DATABASE_URI'])).get()
